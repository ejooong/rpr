...
<x-app-layout>
    ...
    <div class="py-6">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-8">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800">Rekomendasi Komoditas Per Sektor</h3>
                    <p class="text-sm text-gray-600 mt-1">
                        @if($kabupatenId)
                            Rekomendasi untuk <span class="font-semibold">{{ $kabupatens->where('id', $kabupatenId)->first()->nama ?? 'Tidak Diketahui' }}</span>
                            @if($provinsiId)
                            ...
                            @endif
                        @else
                            <span class="text-yellow-600">Pilih kabupaten/kota terlebih dahulu untuk melihat rekomendasi per sektor</span>
                        @endif
                        - Tahun {{ $tahun }}
                    </p>
                </div>
                <button id="refreshRekomendasiSektor" 
                        class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md border border-transparent text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition duration-150 ease-in-out"
                        {{ !$kabupatenId ? 'disabled' : '' }}>
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                    </svg>
            ...
            <div id="rekomendasiSektorLoading" class="hidden text-center py-8">
                <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600 mx-auto"></div>
                <p class="mt-2 text-gray-600">Memuat rekomendasi per sektor...</p>
            </div>

            <div id="rekomendasiSektorContent">
                @if(!$kabupatenId)
                <!-- Pesan jika kabupaten belum dipilih -->
                <div class="text-center py-8 text-yellow-600">
                    <svg class="w-12 h-12 mx-auto text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <p class="mt-2">Silakan pilih kabupaten/kota terlebih dahulu untuk melihat rekomendasi per sektor</p>
                    <p class="text-sm mt-1">Pilih provinsi kemudian kabupaten/kota pada filter di atas</p>
                </div>
                @else
                <!-- Default content sebelum di-load -->
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="mt-2">Klik "Refresh Per Sektor" untuk melihat rekomendasi komoditas berdasarkan sektor</p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-8">
            <div class="flex justify-between items-center mb-4">
                ...
                <span class="text-sm text-gray-500">Top 5 Produk Unggulan</span>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="border border-gray-200 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($produk['detail_wilayah']->take(5) as $wilayah)
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-8">
            <div class="flex justify-between items-center mb-6">
                ...
                <span class="text-sm text-gray-500">Top 5 Berdasarkan Produktivitas</span>
            </div>
    ...
    fetch(`/bps-data/rekomendasi-per-sektor?${params}`)
        ...
        function renderRekomendasiSektorContent(data) {
            ...
            if (!data.rekomendasi_per_sektor || Object.keys(data.rekomendasi_per_sektor).length === 0) {
                contentElement.innerHTML = `
                    <div class="text-center py-8 text-gray-500">
                        <svg class="w-12 h-12 mx-auto text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.47-.927-6.04-2.444M3 15a9 9 0 1118 0 9 9 0 01-18 0z"></path>
                        </svg>
                        <p class="mt-2">Tidak ada rekomendasi komoditas per sektor untuk wilayah ini</p>
                        <p class="text-sm mt-1">Coba ubah filter tahun atau pilih kabupaten lain</p>
                    </div>
                `;
                return;
            }
            ...
            let html = `
                <div class="mb-4 p-3 bg-purple-50 border-purple-200 border rounded-lg">
                    <div class="flex items-center">
                        <svg class="w-4 h-4 text-purple-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <div class="text-sm text-purple-700">
                            <span class="font-medium">Rekomendasi Per Sektor:</span>
                            <span class="ml-1">${getWilayahInfo(data.filter, 'kabupaten')}</span>
                        </div>
                    </div>
                </div>
            `;
            ...
            Object.entries(data.rekomendasi_per_sektor).forEach(([namaSektor, rekomendasiSektor]) => {
                const sektorColor = getSektorColor(namaSektor);
                
                html += `
                    <div class="mb-6 border border-gray-200 rounded-lg">
                        <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                            <h4 class="font-semibold text-gray-800 flex items-center">
                                <span class="w-3 h-3 rounded-full mr-2 ${sektorColor}"></span>
                                ${namaSektor}
                                <span class="ml-2 text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded-full">
                                    ${rekomendasiSektor.length} komoditas teratas
                                </span>
                            </h4>
                        </div>
                ...
                rekomendasiSektor.forEach((item, index) => {
                    const badgeColor = getBadgeColor(item.rekomendasi_level);
                    const rankClass = index < 2 ? 'ring-2 ring-yellow-400' : '';
                    
                    html += `
                        <div class="border border-gray-200 rounded-lg p-4 ${rankClass} hover:shadow-md transition-shadow duration-200">
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex items-center">
                                    <span class="flex items-center justify-center w-6 h-6 rounded-full bg-purple-100 text-purple-800 font-bold text-xs mr-2">
                                        ${index + 1}
                                    </span>
                                    <div>
                                        <h5 class="text-sm font-semibold text-gray-800">${item.nama}</h5>
                                        <p class="text-xs text-gray-500">${item.sektor}</p>
                                    </div>
                                </div>
                                <span class="px-2 py-1 text-xs font-medium rounded-full ${badgeColor}">
                                    ${item.rekomendasi_level}
        ...
        function getSektorColor(sektorName) {
            const colors = {
                'Hortikultura': 'bg-green-500',
                'Tanaman Pangan': 'bg-yellow-500',
                'Perkebunan': 'bg-orange-500',
                'Peternakan': 'bg-red-500',
                'Perikanan': 'bg-blue-500'
            };
