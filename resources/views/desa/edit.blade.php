...
<x-app-layout>
    <x-slot name="header">
        ...
        <p class="text-sm text-gray-600 mt-1">Ubah data desa dan kelurahan</p>
    </x-slot>
                                ...
                                <select name="provinsi_id" 
                                    ...
                                    @foreach($provinsis as $provinsi)
                                        <option value="{{ $provinsi->id }}" 
                                            {{ old('provinsi_id', $desa->kecamatan->kabupaten->provinsi_id) == $provinsi->id ? 'selected' : '' }}>
                                            {{ $provinsi->nama }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="kabupaten_id" 
                                    ...
                                    @foreach($kabupatens as $kabupaten)
                                        <option value="{{ $kabupaten->id }}" 
                                            ...
                                            {{ old('kabupaten_id', $desa->kecamatan->kabupaten_id) == $kabupaten->id ? 'selected' : '' }}>
                                            {{ $kabupaten->nama }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="flex space-x-4">
                                    <label class="inline-flex items-center">
                                        <input type="radio" 
                                               name="tipe" 
                                               value="desa" 
                                               {{ old('tipe', $desa->tipe) == 'desa' ? 'checked' : '' }}
                                    ...
                                    <label class="inline-flex items-center">
                                        <input type="radio" 
                                               name="tipe" 
                                               value="kelurahan" 
                                               {{ old('tipe', $desa->tipe) == 'kelurahan' ? 'checked' : '' }}
                                               class="text-blue-600 focus:ring-blue-500 border-gray-300">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-6 border border-red-200">
                <div class="p-6">
                    ...
                    <p class="text-sm text-red-600 mb-4">
                        Hapus permanen data desa/kelurahan. Aksi ini tidak dapat dibatalkan.
                    </p>
                ...
                <div class="flex">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-yellow-700 mt-1">
                            Desa/kelurahan ini masih memiliki data terkait:
                            @php
                                $relatedData = [];
                                if ($desa->petani()->count() > 0) $relatedData[] = $desa->petani()->count() . ' petani';
                                if ($desa->poktan()->count() > 0) $relatedData[] = $desa->poktan()->count() . ' kelompok tani';
                                if ($desa->demplot()->count() > 0) $relatedData[] = $desa->demplot()->count() . ' demplot';
                            @endphp
                            {{ implode(', ', $relatedData) }}.
                        </p>
                    </div>
    ...
    <script>
        ...
        function updateKecamatan() {
            ...
            // Show only kecamatan from selected kabupaten
            ...
            // Reset selection if current selection doesn't match
            const currentKecamatan = kecamatanSelect.value;
            if (currentKecamatan) {
                const selectedOption = kecamatanSelect.querySelector(`option[value="${currentKecamatan}"]`);
                if (selectedOption && selectedOption.style.display === 'none') {
