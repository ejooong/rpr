<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Sektor: {{ $sektor->nama }}
        </h2>
        <p class="text-sm text-gray-600 mt-1">Edit data sektor komoditas</p>
    </x-slot>
                    ...
                    <form action="{{ route('sektor.update', $sektor->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="kode" id="kode" value="{{ old('kode', $sektor->kode) }}" 
                            ...
                            <div>
                                ...
                                <input type="text" name="nama" id="nama" value="{{ old('nama', $sektor->nama) }}" 
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="deskripsi" id="deskripsi" rows="3"
                                          class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $sektor->deskripsi) }}</textarea>
                                @error('deskripsi')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                <div class="flex items-center">
                                    <input type="checkbox" name="aktif" id="aktif" value="1" 
                                           {{ old('aktif', $sektor->aktif) ? 'checked' : '' }}
